...
namespace GwanKei {
  ...
  */
  ...
  bool Bound::is_linkable(const Bound& prev, bool able_to_turn = false) const {
  ...
  Bound& Bound::operator = (const Bound& right) {
    ...
    left.railway_orient_origin = right.railway_orient_origin;
    left.railway_orient_terminal = right.railway_orient_terminal;
    return left;
  }
  ...
  std::list<Cell> get_route(
    ...
    stack.push(SearchNode(inital_bound, initial_route));

    while(!stack.empty()) {
      SearchNode node = stack.top();
      stack.pop();
      visited[node.bound.target.get_id()] = true;

      if(node.bound.target == to)
	return node.route;
      
      std::list<Bound> new_bounds = node.bound.target.get_adjacents();
      for(auto I=new_bounds.begin(); I!=new_bounds.end(); I++) {
	Bound& new_bound = *I;
	if(!occupy_state[new_bound.target.get_id()]
	   && !visited[new_bound.target.get_id()]
	   && new_bound.is_linkable(node.bound, able_to_turn)
	   ) {
	  std::list<Cell> route = node.route;
	  route.push_back(new_bound.target);
	  stack.push(new_bound, route);
	}
      }
      visited[node.bound.target.get_id()] = false;
    } // while not empty
